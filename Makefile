PATH=/usr/bin:/opt/sun-jdk-1.5.0.06/bin/:/bin
...
ANGELO = /users/u12/adinardi/code/newDrink

# BDM & SNACK
TINICODE = $(ANGELO)/tini1.02h/
TINICONVLINE = java -classpath $(ANGELO)/tini1.02h/bin/tini.jar BuildDependency -d $(ANGELO)/tini1.02h/bin/tini.db -add OneWireContainer05 -add OneWireContainer28 -add OneWireMonitor -x $(ANGELO)/tini1.02h/bin/owapi_dep.txt -p $(ANGELO)/tini1.02h/bin/owapi_dependencies_TINI.jar 
...
commlinktest: compile
	$(TINICONVLINE) -f CollectGarbage.class -f OneWireCommands.class -f OneWireLights.class -f SlotMonitorListener.class -f CommLink.class -f IncomingLink.class -f OutgoingLink.class -f ConfigMgr.class -f Starter.class -f TempWorker.class -f NoOpWorker.class -m Starter -o Starter.tini 
clean: 
	rm *.class *.tini
